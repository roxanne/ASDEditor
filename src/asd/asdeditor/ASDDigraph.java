...
/** 
   ...
   @version 1.03 2001 Oct-Nov; 2003 Jul; 2005 Oct; 2010 Aug 
 */ 
...
   /** 
   ...
   {  ASDDigraph copy = shallowCopyDigraph(); 
      ArrayList<ASDDigraphNode> initials 
         = new ArrayList<ASDDigraphNode>(numberOfNodes()); 
      ArrayList<ASDDigraphNode> finals 
         = new ArrayList<ASDDigraphNode>(numberOfNodes()); 
      for (Iterator<DigraphNode> it 
         = copy.getNodes().iterator(); it.hasNext(); ) 
      {  ASDDigraphNode dNode = (ASDDigraphNode) it.next(); 
         ASDGrammarNode gNode = dNode.getGrammarNode(); 
         if (gNode.isInitial()) 
            initials.add(dNode); 
      ...
      // final node ends: 
      for (Iterator<ASDDigraphNode> it 
         = finals.iterator(); it.hasNext(); ) 
      {  ASDDigraphNode dNode = it.next(); 
         ASDGrammarNode gNode = dNode.getGrammarNode(); 
         String type = gNode.phraseType(); 
         for (Iterator<ASDDigraphNode> it2 
              = initials.iterator(); it2.hasNext(); ) 
         {  ASDDigraphNode dNode2 = it2.next(); 
...
   void mergeInGrammar(ASDGrammar otherGrammar, Container panel) 
      throws IOException, 
             ClassNotFoundException, InvocationTargetException, 
             InstantiationException, IllegalAccessException 
   {  HashMap<String, ArrayList<ASDGrammarNode>> otherLexicon 
         = otherGrammar.lexicon(); 
      Set<Map.Entry<String, ArrayList<ASDGrammarNode>>> otherEntrySet 
         = otherLexicon.entrySet(); 
      ...
      for (Iterator<Map.Entry<String, ArrayList<ASDGrammarNode>>> it 
           = otherEntrySet.iterator(); it.hasNext(); ) 
      {  Map.Entry<String, ArrayList<ASDGrammarNode>> e 
            = (Map.Entry<String, ArrayList<ASDGrammarNode>>) it.next(); 
         String word = (String)e.getKey(); 
         ArrayList<ASDGrammarNode> instances = e.getValue(); 
         ArrayList<ASDGrammarNode> wordEntry 
            = grammar.lookupWord(word); 
         if (wordEntry == null) // new word for the current grammar 
         {  wordEntry = new ArrayList<ASDGrammarNode>(instances.size()); 
            grammar.lexicon().put(word, wordEntry); 
         } 
         int increment = wordEntry.size(); 
         for (Iterator<ASDGrammarNode> j 
              = instances.iterator(); j.hasNext(); ) 
         {  ASDGrammarNode oldGNode = j.next(); 
            if (increment > 0) 
            ...
            {  ArrayList<ASDGrammarSuccessor> successors 
                  = oldGNode.successors(); 
               for (Iterator<ASDGrammarSuccessor> k 
                    = successors.iterator(); k.hasNext(); ) 
               {  ASDGrammarSuccessor s = k.next(); 
                  String sWord = s.getWord(); 
                  ArrayList<ASDGrammarNode> sWordEntry 
                     = grammar.lookupWord(sWord); 
...
      // Now add the old grammar entries to the current grammar and 
      ...
      ArrayList<ASDDigraphNode> newNodes 
         = new ArrayList<ASDDigraphNode>(); 
      for (Iterator<Map.Entry<String, ArrayList<ASDGrammarNode>>> it 
            = otherEntrySet.iterator(); it.hasNext(); ) 
      {  Map.Entry<String, ArrayList<ASDGrammarNode>> e = it.next(); 
         String word = (String)e.getKey(); 
         ArrayList<ASDGrammarNode> newInstances 
            = grammar.lookupWord(word); 
         ArrayList<ASDGrammarNode> oldInstances 
            = e.getValue(); 
         for (Iterator<ASDGrammarNode> j 
            = oldInstances.iterator(); j.hasNext(); ) 
      ...
      // node connects to: 
      for (Iterator<ASDDigraphNode> it = newNodes.iterator(); it.hasNext(); ) 
      {  ASDDigraphNode dNode = it.next(); 
         ASDGrammarNode gNode = dNode.getGrammarNode(); 
         if ( !gNode.isFinal() ) 
         {  ArrayList<ASDGrammarSuccessor> successors = gNode.successors(); 
            for (Iterator<ASDGrammarSuccessor> j 
                 = successors.iterator(); j.hasNext(); ) 
            {  ASDGrammarSuccessor s = j.next(); 
               ASDGrammarNode nextgNode = grammar.lookupInstance(s); 
               ASDDigraphNode nextdNode = null; 
               for (Iterator<ASDDigraphNode> k 
                    = newNodes.iterator(); k.hasNext(); ) 
               {  ASDDigraphNode d = k.next(); 
                  if (d.getGrammarNode() == nextgNode) 
                  {  nextdNode = d; 
                     break; 
                  } 
...
   void removeEdge(ASDDigraphEdge e) 
   ...
   {  if (n == null) return; 
      ...
      int instanceIndex = 1; 
      for (Iterator<ASDGrammarNode> it 
...
   /** 
   ...
   {  // Open a string for the given file name: 
      ...
      ArrayList<ASDWordEntry> entryList 
         = new ArrayList<ASDWordEntry>(grammar.lexicon().size()); 
 
      Set<Map.Entry<String, ArrayList<ASDGrammarNode>>> entrySet 
         = grammar.lexicon().entrySet(); 
      for (Iterator<Map.Entry<String, ArrayList<ASDGrammarNode>>> it 
      ...
      {  ASDWordEntry entry = (ASDWordEntry) sortedList[j]; 
         String word = (String) entry.getWord(); 
         ArrayList<ASDGrammarNode> instances 
            = (ArrayList<ASDGrammarNode>) entry.getInstances(); 
...
   /** 
   ...
   {  ASDDigraph result = new ASDDigraph(); 
      ...
      ArrayList<DigraphNode> dNodes = getNodes(); 
      for (Iterator<DigraphNode> it = dNodes.iterator(); it.hasNext(); ) 
      ...
      ArrayList<DigraphEdge> edges = getEdges(); 
      for (Iterator<DigraphEdge> it = edges.iterator(); it.hasNext(); ) 
      {  ASDDigraphEdge edge = (ASDDigraphEdge) it.next(); 
         ...
         int fromNodeIndex = -1; 
         for (Iterator<DigraphNode> nodeIt 
            = dNodes.iterator(); nodeIt.hasNext(); ) 
