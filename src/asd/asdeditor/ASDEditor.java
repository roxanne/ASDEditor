...
import asd.asdeditor.ListDialog;
import asd.asdeditor.ASDGrammar;
import asd.asdeditor.ASDGrammarNode;
import asd.asdeditor.ASDInputException;
import java.io.*;
import java.net.*;
import java.util.*;
...
<BR>java asd/ASDEditor grammarFileName</b></tt>
   ...
   @version 1.08  2003 Jul
 */
public class ASDEditor extends Thread
{  /**
...
//      fileChooser = new FileSelectMenu(this);
//  java.awt.Component.setLocationRelativeTo(this);
...
//      currentDirectory = new File("", "");
         // root directory of disk from which editor was invoked
//      currentDirectory = new File(File.pathSeparator);
      ...
      window.setTitle(
         WINDOW_TITLE
//            + currentDirectory.toString() + File.separator
            + inputFileName);
...
//      eEdge.setEditor(this); // done by netPanel.addEdge
...
//      eNode.setEditor(this);  // redundant; done by netPanel.addNode
...
//            window.newOrUpdatedGrammar(false);
   ...
   String[] getDemoFileNames(String webAddress)
   {  Vector fileNames = new Vector();
      InputStream stream = null;
      BufferedReader reader;
      try {
         URL inputSource = new URL(webAddress);
         stream = inputSource.openStream();
      ...
      for (int j = 0; j < fileNames.size(); ++j)
         fileNameArray[j] = (String) fileNames.elementAt(j);
      return fileNameArray;
   }  // end getDemoFileNames
 ...
 */
      ...
      try
      {  grammarDigraph
         = new ASDDigraph(currentDirectory + inputFileName, netPanel);
      }
      catch (ASDInputException e)
      {  JOptionPane.showMessageDialog(window, e.getMessage(),
            "Syntax error in grammar file " + inputFileName,
 ...
 */
   ...
   {  // If the previous grammar has been changed, prompt for
      ...
      if (grammarChanged) // Give user the option of saving the
         ...
         oldGrammarClosed = closingGrammar();
      if (oldGrammarClosed)
      {  // Get the list of demo files from asddemo web site:
         String[] fileNameArray = getDemoFileNames(currentDirectory +
                                                   demoFileNames);

         // Present the ListDialog for user selection:
         ListDialog.initialize(
            window, fileNameArray, "Open file:", "");
         String inputFileChosen
            = ListDialog.showDialog(window, null);

         if (inputFileChosen != null)
            loadFile(inputFileChosen);
   ...
   {  boolean successfulLoad = true;
      try
      {  grammarDigraph.mergeInGrammar(currentDirectory + fileName,
                                       netPanel);
      }
      catch (ASDInputException e)
      {  JOptionPane.showMessageDialog(window, e.getMessage(),
   ...
   {  if (!node.isSelected()
         ...
         // click it down.
      {
           nodeSelected(node);
//         node.doClick();
            // doClick() must be used here, because
            // when this method is invoked by edgeSelected, the
            // following code is NOT enough to update the instance list:
   ...
   static String readLine(BufferedReader inputReader)
...
//      catch(FileNotFoundException e)
   ...
   {  netPanel.clear();
      ...
      {  ASDDigraphNode dNode = (ASDDigraphNode) it.next();
         ASDEditNode eNode = dNode.getEditNode();
//         eNode.setEditor(this);  reduntant; done by netPanel.addNode
         netPanel.addNode(eNode);
      }
      ...
      {  ASDDigraphEdge dEdge = (ASDDigraphEdge) it.next();
         ASDEditEdge eEdge = dEdge.getEditEdge();
//         eEdge.setEditor(this);  redundant; done by netPanel.addEdge
         netPanel.addEdge(eEdge);
      }
//      window.netPanelChanged();
   }
...
//         eNode2.setEditor(this);
         ...
         {  netPanel.addNode(eNode2);
            ...
            {  ASDDigraphEdge dEdge = (ASDDigraphEdge)eIt.next();
               ASDEditEdge eEdge = dEdge.getEditEdge();
//               eEdge.setEditor(this);
//               if (!netPanel.isDisplaying(eEdge)) // redundant
                  netPanel.addEdge(eEdge);
            }
            ArrayList inEdges = dNode2.getInEdges();
            for (Iterator eIt = inEdges.iterator(); eIt.hasNext(); )
...
//               if (!netPanel.isDisplaying(eEdge)) // redundant
   ...
   {  ASDGrammarNode gNode = eNode.getGrammarNode();
      ...
      if (eNodeSelected1 == eNode)
         window.edgeListChanged();
//         window.newOrUpdatedGrammar(false);
// Neither of these repaints is needed, because ASDEditNode setRightLabel
// will have repainted the changed node already:
//      netPanel.repaint();  // or window.netPanelChanged() ?
//      eNode.repaint();
      setGrammarChanged(true);
      return true;
   } // end toggleFinal
...
//      "http://www.cs.yorku.ca/~jmason/asddemo/";
       "http://www.asdnetworks.com/grammars/";  
   private String demoFileNames = "demoGrammarFiles.txt";
   private String inputFileName;
   private String inputFileChosen;
   static final String VERSION = "1.08";
   static final String WINDOW_TITLE = "ASD Grammar Editor (v.1.08): ";
